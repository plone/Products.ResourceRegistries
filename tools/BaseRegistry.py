...
class BaseRegistryTool(UniqueObject, SimpleItem, PropertyManager):
    ...
    def unregisterResource(self, id ):
        ...
        resources = [ item for item in self.getResources() if item.get('id') != id ]
    ...
    def getResources(self ):
        """ get all the registered resource data, uncooked. for management screens """
    ...
    def clearResources(self):
        """ Clears all resource data. Convenience funtion for Plone migrations and tests"""
        self.resources = ()
        self.cookedresources = ()
        self.concatenatedresources = {}
    ...
    def getResourcesDict(self):
        """ get the resources as a disctionary instead of an ordered list. Good for lookups. internal"""
    ...
    def cookResources(self ):
        ...
        for resource in resources:
            if results:
                previtem = results[-1]
                if not self.getDebugMode() and self.compareResources(resource, previtem):
                    previd = previtem.get('id')

                    self.finalizeResourceMerging(resource, previtem)

                    if self.concatenatedresources.has_key(previd):
                        self.concatenatedresources[previd].append(resource.get('id'))
                    else:
                        magicId = self.generateId()
                        self.concatenatedresources[magicId] = [previd, resource.get('id')]
                        previtem['id'] = magicId
                else:
    ...
    def getResource(self, item, context):
        ...
        for id in ids:
            ...
            # add start/end notes to the resource
    ...
    def __bobo_traverse__(self, REQUEST, name):
        """ traversal hook"""
        if REQUEST is not None and self.concatenatedresources.get(name,None) is not None:
